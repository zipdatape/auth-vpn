...
clear
echo -e "${AZUL}=== Script de instalación y configuración de Azure VPN y Radius ===${NC}\n"
...
if [ ! -d "azure-vpn" ]; then
    ...
    git clone https://github.com/zipdatape/azure-vpn.git > /dev/null 2>&1
...
if [ ! -d "$RADIUS_CONTAINER_NAME" ]; then
    ...
    git clone https://github.com/zipdatape/azure_radius.git "$RADIUS_CONTAINER_NAME" > /dev/null 2>&1
